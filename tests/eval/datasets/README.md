...
For this capstone, there are two eval layers:

1. `agents-cli eval` assets for the ADK shell agent.
2. `tests/eval/run_first_slice_eval.py` for the actual first-slice review API path.
3. `tests/eval/run_evidence_outcome_eval.py` for the compact live ADK verdict gate.

The second one matters more right now, because the real product behavior lives in the structured review endpoints, not in a free-form chat agent.

### Compact live verdict gate

```bash
python tests/eval/run_evidence_outcome_eval.py
```

This runs four bounded evidence packets through the same tool-free ADK Agent,
Runner, schema, and short-lived session used by the product. Its JSON output
preserves expected/actual verdicts, reasons, failure categories, elapsed time,
and token usage without requiring a GCP project.

## Running Evaluations

### First-slice workflow harness

```bash
python tests/eval/run_first_slice_eval.py
```

What it does:
- runs deterministic first-slice cases now
- skips live Gemini semantic cases until `.env` contains a real key
- prints a JSON result summary with pass/skip/fail per case
